...
const cmad = decodeURIComponent(atob('JUU4JUJGJTk5JUU0JUI4JUFBJUU1JUIwJTkxJUU1JUI5JUI0JUU2JTg0JUJGJUU3JTk0JUE4JUU0JUI4JTgwJUU3JTk0JTlGJUU1JUJEJUI3JUU1JUJFJUE4JUU0JUI4JUJBJUU2JTgyJUE4JUU2JThDJTg3JUU1JUJDJTk1JUU4JTg0JTlBJUU0JUI4JThCJUU3JTlBJTg0JUU4JUI3JUFGJTdF'));
/**
 * @param {string} userID
...
async function 生成配置信息(userID, hostName, sub, UA, RproxyIP, _url, fakeUserID, fakeHostName, env) {
    ...
    if (userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
        ...
        let 订阅器 = '<br>';
        if (sub) {
            if (enableSocks) 订阅器 += `（访问方式）: ${enableHttp ? "HTTP" : "Socks5"}<br>&nbsp;&nbsp;${newSocks5s.join('<br>&nbsp;&nbsp;')}<br>${socks5List}`;
            else if (proxyIP && proxyIP != '') 订阅器 += `（访问方式）: ProxyIP<br>&nbsp;&nbsp;${proxyIPs.join('<br>&nbsp;&nbsp;')}<br>`;
            else if (RproxyIP == 'true') 订阅器 += `（访问方式）: 自动获取ProxyIP<br>`;
            else 订阅器 += `（访问方式）: 内置兜底, 您也可以设置 proxyIP/PROXYIP 。<br>`
